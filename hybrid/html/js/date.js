...
// 获取:年月日，''格式：yyyyMMdd，'-'格式：yyyy-MM-dd
 function getDateYMD(date,seperator='-') {
...
// 获取：年月日时，''格式：yyyyMMddHH，'-'格式：yyyy-MM-dd HH
 function getDateYMDHM(date,seperator='-') {
  var month = date.getMonth() + 1;
  var strDate = date.getDate();
  ...
  if(seperator==='-')
  {
    currentdate=date.getFullYear() + seperator + addZero(month) + seperator + addZero(strDate)+ ' ' + addZero(hour) + ':'+ addZero(Minutes);
  }
  else
    {
    currentdate=date.getFullYear() + seperator + addZero(month) + seperator + addZero(strDate)+ seperator + addZero(hour) + addZero(Minutes);
  }
  return currentdate;
};
// 获取：年月日时，''格式：yyyyMMddHH，'-'格式：yyyy-MM-dd HH
function getDateYMDH(date,seperator='-') {
  var month = date.getMonth() + 1;
  var strDate = date.getDate();
  var hour = date.getHours();
  var currentdate =''
...
//获取：年月日时分秒 ，''格式：yyyyMM，'-'格式：yyyy-MM
 function getDateYM(date,seperator='-') {
    var date = date;
    var month = date.getMonth() + 1;
    var strDate = date.getDate();
    var hour = date.getHours();
...
//获取：年月日时分秒 ，''格式：yyyyMMddHHmmss，'-'格式：yyyy-MM-dd HH:mm:ss
 function getDateYMDHMS(date,seperator='-') {
    var date = date;
    var month = date.getMonth() + 1;
    var strDate = date.getDate();
    var hour = date.getHours();
...
/* 返回中文日期， yyyy年MM月dd日HH时mm分ss秒 */
 function getDateYMDHMS_CN(date) {
  var month = date.getMonth() + 1;
  var strDate = date.getDate();
  var hour = date.getHours();
  var minute = date.getMinutes();
...
/* 返回中文日期， HH时mm分 */
function getDateDHM_CN(date) {
    var strDate = date.getDate();
    var hour = date.getHours();
    var minute = date.getMinutes();
    var currentdate =strDate + '日'+hour+ '时' + minute + '分';
    return currentdate;
};
/* 返回中文日期， HH时 */
function getDateDH_CN(date) {
    var strDate = date.getDate();
    var hour = date.getHours();
    var currentdate =strDate + '日'+hour+ '时';
    return currentdate;
};
/* 对日期进行加减 */
 function dateAdd(date,type,number)
{
  ...
  if(type==='f') date.setMinutes(date.getMinutes() + number);
  return date;
}
...
/* 2个日期字符串相减，返回相差小时数,日期参数格式yyyyMMddHH格式*/
 function DateDiffHour(sDate1, sDate2) {
...
//获取月份的最后一天返回天数
function getLastDateDay(date) {
  //取当下月中的第一天
  let lastDate = getLastDateOfMonth(date)
  return lastDate.getDate();
}
/**
 * 给时间对象一个属性Format，用于格式化时间
 * 
 * @param fmt
 * @returns {*}
 * @constructor
